...
def extract_prompt_prices(models: list[dict]) -> dict[str, str]:
    prices = {}
    for model in models:
        ...
        if not should_include_model(base_name):
            continue
        if base_name not in prices or prompt_price < Decimal(prices[base_name]):
            prices[base_name] = str(prompt_price)
...
def format_str_dict(items: dict[str, str], indent: int = 4) -> str:
    """Format a string dict as Python code."""
    if not items:
        return "{}"
    prefix = " " * indent
    lines = [f'{prefix}"{k}": "{v}",' for k, v in items.items()]
...
OPENROUTER_MODEL_PROMPT_PRICES = {format_str_dict(prompt_prices)}
...
def main():
    ...
    prompt_prices = extract_prompt_prices(models)
    print(f"Prompt prices extracted: {len(prompt_prices)}")
