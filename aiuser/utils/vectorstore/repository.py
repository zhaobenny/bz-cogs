...
class Repository:
    def __init__(self, cog_data_path: str):
    ...
    async def upsert(self, guild_id: int, memory_name: str, memory_text: str, last_updated: int) -> int:
        """Insert a new memory row. Returns number of rows in table after insert. """
        await self._connect()

        table = await self.db.open_table(MEMORY_TABLE_NAME)

        embedding = await embed_text(memory_text, self.cog_data_path)
    ...
    async def fetch_by_rowid(self, rowid: int, guild_id: int) -> Optional[Tuple[str, str]]:
    ...
    async def delete(self, rowid: int, guild_id: int) -> bool:
        ...
        if idx < 0 or idx >= len(rows):
            return False
        target = rows[idx]
        global_rowid = target.get("_rowid_")
        if global_rowid is None:
            return False

        await table.delete(f"_rowid_ = {int(global_rowid)}")
        return True

    async def search_similar(self, query: str, guild_id: int, k: int = 1) -> List[Tuple[str, str, float]]:
