...
async def remove_patterns_from_response(ctx: commands.Context, config: Config, response: str) -> str:
    ...
    # === ADDITION: Blacklist phrases ===
    # Pull blacklisted phrases from guild config
    blacklist_phrases = await config.guild(ctx.guild).removelist_regexes() or []
    for phrase in blacklist_phrases:
        expanded_patterns.append(re.escape(phrase))
