...
class AIEmote(commands.Cog, Settings):
    ...
    async def pick_emoji(self, message: discord.Message):
        ...
        if not emojis:
            logger.warning(
                f"Skipping react! No valid emojis to use in {message.guild.name}"
            )
            return None
    ...
    async def is_valid_to_react(self, ctx: commands.Context):
        ...
        if isinstance(ctx.channel, discord.Thread) and (
            ctx.channel.parent_id not in whitelist
        ):
            return False
        ...
        if len(ctx.message.content) > 1500 or len(ctx.message.content) < 10:
            logger.debug(
                f"Skipping message in {ctx.guild.name} with length {len(ctx.message.content)}"
            )
            return False
        ...
        for mentioned in mentions:
            if mentioned in message.channel_mentions:
                ...
                content = content.replace(
                    mentioned.mention, f"@{mentioned.display_name}"
                )
