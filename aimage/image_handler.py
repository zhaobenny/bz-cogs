...
class ImageHandler(MixinMeta):
    async def _execute_image_generation(
        ...
        if self.generating[user.id]:
            content = _(
                ":warning: You must wait for your current image to finish generating before you can request a new one."
            )
        ...
        if await self._contains_blacklisted_word(guild, prompt):
            return await send_response(
                context, content=_(":warning: Prompt contains blacklisted words!")
            )
        ...
        try:
            ...
            return await send_response(
                context,
                content=_(":warning: This method is not supported by the host!"),
                ephemeral=True,
            )
        except Exception:
            logger.exception(
        ...
        if response.is_nsfw and not await self.config.guild(guild).allow_nsfw():
            return await send_response(
                context,
                content=_(
                    "🔞 {user_mention} generated a possible NSFW image with prompt: `{prompt}`"
                ).format(user_mention=user.mention, prompt=prompt),
