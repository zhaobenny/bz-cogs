...
from aimage.common.constants import (ADETAILER_ARGS, A1111_SAMPLERS,
                                     TILED_VAE_ARGS)
...
class ImageHandler(MixinMeta):
    ...
    async def generate_image(self, context: Union[commands.Context, discord.Interaction], params: ImageGenParams):
        payload = await self._generate_payload(context.guild, params)
        await self._execute_image_generation(context, params, payload, ImageGenerationType.TXT2IMG)

    async def generate_img2img(self, context: discord.Interaction, params: ImageGenParams):
        ...
        await self._execute_image_generation(context, params, payload, ImageGenerationType.IMG2IMG)
    ...
    async def _execute_image_generation(self,
        ...
        try:
            ...
            response: ImageResponse = await self._post_image_gen(guild, payload, generation_type)
        ...
        if response.is_nsfw and not await self.config.guild(guild).nsfw() and not await self.config.guild(guild).nsfw_blurred():
            return await send_response(context, content=f"🔞 {user.mention} generated a possible NSFW image with prompt: `{params.prompt}`", allowed_mentions=discord.AllowedMentions.none())
    ...
    async def _post_image_gen(self, guild: discord.Guild, payload: dict, generation_type: ImageGenerationType):
    ...
    async def _update_autocomplete_cache(self, ctx: Union[commands.Context, discord.Interaction]):
        ...
        for page, cache_key in cache_mapping.items():
            try:
                ...
                logger.warning(f"Failed to update autocomplete cache for {cache_key} in {ctx.guild.id}: \n {e}")
                continue
