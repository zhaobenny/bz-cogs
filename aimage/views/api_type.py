...
class APITypeSelect(Select):
    def __init__(self, config: Config, ctx: commands.Context):
        options = [
            discord.SelectOption(label=api_type.name, value=api_type.value)
            for api_type in API_Type
        ]
        self.ctx = ctx
        self.config = config
        super().__init__(placeholder="Type of API", options=options)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Selected`{selected_value}`", ephemeral=True
        )
